...
class DEBLUR(object):
    def __init__(self, args):
        ...
        self.train_dir = os.path.join('./checkpoints-mod2', args.model)  ###
        ...
        with tf.variable_scope(scope, reuse=reuse):
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                ...
                for i in xrange(self.n_levels):
                    ...
                    if self.args.model == 'lstm':
                        deconv3_2, rnn_state = cell(conv3_2, rnn_state)
                    else:
                        deconv3_2 = conv3_2

                    # decoder
                    deconv3_1 = ResNetBlock(deconv3_2, 128, 5, scope='dec3_1')
                    deconv2_2 = slim.conv2d_transpose(deconv3_1, 64, [4, 4], stride=2, scope='dec2_1')
                    cat2 = deconv2_2 + conv2_2
                    deconv2_1 = ResNetBlock(cat2, 64, 5, scope='dec2_1')
                    deconv1_2 = slim.conv2d_transpose(deconv2_1, 32, [4, 4], stride=2, scope='dec1_4')
                    cat1 = deconv1_2 + conv1_2
                    deconv1_1 = ResNetBlock(cat1, 32, 5, scope='dec1_1')
    ...
    def train(self):
        file = open('./log-mod-2-' + self.args.model + '.txt', 'w')
...
#             train_op = tf.train.GradientDescentOptimizer(self.lr)
        ...
        for step in xrange(sess.run(global_step), self.max_steps + 1):
            ...
            if step % (self.data_size) == 0:
                ...
                file.write(format_str % (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), step, loss_total_val, 0.0,
    ...
    def test(self, height, width, input_path, output_path):
        ...
        self.load(sess, self.train_dir, step=40000)
