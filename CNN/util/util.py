...
# Modified ResNetBlock
def ResNetBlock(x, dim, ksize, scope='rb'):
    with tf.variable_scope(scope):
        net = slim.conv2d(x, dim, [ksize, ksize], scope='conv1')
        net = slim.conv2d(net, dim, [ksize, ksize], scope='conv2')
        net = slim.conv2d(net, dim, [ksize, ksize], scope='conv3')
        net = slim.conv2d(net, dim, [ksize, ksize], activation_fn=None, scope='conv4')
        return net + x
